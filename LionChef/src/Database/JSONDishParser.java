...
public class JSONDishParser {
	...
	public ArrayList<Dish> getJson() throws FileNotFoundException, IOException, ParseException {
		...
		for (Map.Entry<String, JsonElement> entry : object.entrySet()) {
			...
			for (int i = 0; i < array.size(); i++) {
				...
				dishes.add(DishCreater.createDish(name.toLowerCase(), url.toLowerCase()));
			}
		}
		return dishes;
	}
	...
	public void insertJson(ArrayList<Dish> dishes) throws IOException {
		...
		for(int i = 0; i < dishes.size(); i++){
			...
			newObject.addProperty("name", dishes.get(i).getName());
			newObject.addProperty("url", dishes.get(i).getUrl());	
